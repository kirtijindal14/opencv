...
if API_MODULES:
    ...
    # Sibling tree of per-sample example pages — written by
    # `_generate_example_pages()` alongside the class stubs. Each page is
    # `:orphan:`, so it's not in any toctree; the only inbound links are the
    # `<a href="../examples/<name>.html">` references baked into class pages
    # by `_render_examples_block`. Without this glob Sphinx's include-pattern
    # filter would silently drop the entire directory and the Examples links
    # would 404 on every class page.
...
# NOTE: there used to be a ~90-line `_doxygen_xml_to_html` here that walked
# the Doxygen `<detaileddescription>` XML and emitted HTML for each tag
# (programlisting, computeroutput, ref, itemizedlist, …). It was deleted in
# favour of letting Breathe's `{doxygenclass}` directive parse the same XML
# natively — that's what Breathe was built for, and we already depend on it
# elsewhere on the class page. Maintaining our own converter is a trap: any
# new Doxygen tag (tables, notes, warnings, …) would silently drop until we
# notice. See `_write_class_stub`'s "Detailed Description" emission below
# and the `_strip_breathe_class_clutter` build-finished hook which together
...
def _read_class_data(refid: str, xml_dir: pathlib.Path) -> dict | None:
    ...
    # Detailed description: store only a BOOLEAN flag now. The actual
    # rendering is delegated to Breathe's `{doxygenclass} … :no-members:`
    # directive emitted by `_write_class_stub`; that's the same code path
    # Sphinx already uses for every other Doxygen-derived block on the page.
    # We just need to know *whether* there is content so we can decide
    # whether to emit the directive (and the `More...` link in the header).
    detailed_el = cd.find("detaileddescription")
    has_detailed = bool(detailed_el is not None and any(
        _itertext(p).strip() for p in detailed_el.findall("para")
    ))

    # `<includes>` element gives the header file path Doxygen recorded for
    # this class (e.g. `opencv2/core/mat.hpp`) — used by the page header to
    # show the `#include <…>` line, mirroring docs.opencv.org.
...
def _write_class_stub(cls: dict, out_dir: pathlib.Path,
    ...
    # Read class data EARLY so we can emit the include line + one-line brief
    # + "More..." link right under the title (matches docs.opencv.org's
    # class-page header). Falls back to None if the XML wasn't found — the
    # rest of the file then takes its own fallback path below.
    _header_data = _read_class_data(cls["refid"], xml_dir)
    if _header_data is not None:
        import html as _html_pkg
        # Order: brief paragraph FIRST, then the `#include` chip below it.
        # That puts the human-readable description right under the class
        # title and pushes the (more technical) include line down — matches
        # the reading order the user wanted.
        _brief = (_header_data.get("brief") or "").strip()
        if _brief:
            # `More...` link only when there's actually a detailed
            # description further down to navigate to; otherwise a dead jump.
            _more = (
            ...
            lines.append(
                ...
                f'{_html_pkg.escape(_brief)}{_more}'
                f'</p>'
            )
            lines.append("")
        ...
        if _inc:
            lines.append(
                ...
                f'<code>#include &lt;{_html_pkg.escape(_inc)}&gt;</code>'
                f'</div>'
            )
            lines.append("")
    ...
    # `class_simple` is needed both for the Examples lookup below and for
    # the ctor/dtor split further down — compute once.
    class_simple = qualified.rsplit("::", 1)[-1]

    # 2) "Detailed Description" section for the class itself.
    #
    # Delegated to Breathe's `{doxygenclass}` directive so every Doxygen tag
    # the upstream emits (`<programlisting>`, `<computeroutput>`, `<ref>`,
    # `<itemizedlist>`, `<emphasis>`, `<table>`, `<simplesect>`, …) is parsed
    # natively — no custom converter to maintain.
    #
    # `:no-members:` cuts the per-method/per-attribute sections so Breathe
    # produces only the class signature header + brief + detailed body. The
    # signature header and the brief paragraph are duplicates of pieces we
    # already render up top (title + header brief block); they're stripped at
    # build-finished time by `_strip_breathe_class_clutter` below, leaving
    # *only* the detailed-description content under our `<h2>`.
    #
    # (We used to wrap this in a `<div class="opencv-detail-wrapper">`, but
    # MyST silently drops a bare raw-HTML `<div>` block with a directive in
    # between — the wrapper never reached the rendered HTML. The strip hook
    # is now wrapper-free: it scopes to `<section id="detailed-description">`
    # in the output and finds Breathe's `<dl class="cpp class">` directly.)
    #
    # After the directive (still inside the same `## Detailed Description`
    # heading) we append the per-class "Examples" cross-reference. Upstream
    # Doxygen would populate this from EXAMPLE_PATH; we reproduce it here by
    # scanning samples/apps ourselves — see `_find_examples_for_class`.
    examples = _find_examples_for_class(class_simple)
    if data["detailed"]:
        lines += [
            "## Detailed Description",
            "",
            f"```{{doxygenclass}} {qualified}",
            ":project: opencv",
            ":no-members:",
            "```",
            "",
        ...
        # code — give the user the cross-reference anyway under its own
        # heading rather than swallowing the data.
        lines += ["## Examples", ""]
        lines += _render_examples_block(examples)
...
def _generate_api_stubs(modules, xml_dir, out_dir):
    ...
    # Flush the per-sample example pages into a sibling `examples/` directory.
    # The set was populated by `_find_examples_for_class` while each class
    # stub was being written above — generating now keeps us from emitting
    # orphan pages for samples no class actually links to.
...
def _patch_cpp_xref_resolver():
    """Two-part guard for the C++ xref resolver.

    PART 1 — crash guard. Sphinx 8.1.x's `CPPDomain._resolve_xref_inner`
    asserts `parentSymbol`; some breathe-emitted class-page xrefs (e.g.
    `cv::Affine3<T>`) trigger that path with no parent symbol. Treat the
    failure as an unresolved xref (return `(None, None)`) instead of
    crashing the whole build.

    PART 2 — log filter. When Breathe renders the detailed description it
    emits a `pending_xref` for every `<ref>` inside the prose (think
    `cv::Mat`, `cv::transform`, etc.). Many of those don't resolve to a
    symbol declared on our pages (templates, conversion operators, overloads
    Breathe's argument matcher gives up on, …) so the C++ domain prints
    "Unable to resolve function …" / "Cannot find class …" warnings — one
    per miss, dozens per page. The text *still* renders fine (Sphinx falls
    back to a plain `<code>` for the unresolved xref), so the warnings are
    pure noise in our build log. A `logging.Filter` attached to the sphinx
    + docutils loggers drops just those specific message shapes; everything
    else (real errors, missing-document warnings, anonymous-enum noise from
    `_silence_breathe_anon_enum_warning`) flows through untouched.

    We've already exhausted the actionable resolution fixes (the
    `const`-qualifier in `_function_signature`, the `_scoped()`
    class-qualifier in `_write_class_stub`); what remains is genuinely
    unfixable upstream-shape noise. Silencing keeps the build log readable
    without losing any signal we'd actually act on."""
    try:
    ...
    # --- log-filter half: drop the unresolved-xref warning shapes. -------
    import logging
    _UNRESOLVED_XREF_PATTERNS = (
        ...
        "Cannot find define",
        # Breathe sometimes parses the same Doxygen XML twice across an
        # incremental rebuild and re-declares an already-known C++ symbol.
        # The duplicate is harmless (Sphinx ignores the second copy) and
        # was a major source of build-log noise before we cache-busted
        # diagrams via hashed filenames.
...
def _strip_breathe_class_clutter(api_dir: pathlib.Path) -> None:
    ...
    class">` scaffolding on each class page, throwing away the duplicate
    signature header (`<dt>`) and brief paragraph (first `<p>` in `<dd>`).

    Breathe's `{doxygenclass} … :no-members:` emits roughly:
        ...
        <section id="detailed-description">
          ...
          <dl class="cpp class">
            ...
            <dd>
              <p>This is the proxy class for …</p>          ← duplicates header brief
              <p>It is defined as …</p>                     ← what we actually want
              <pre><code>typedef const _InputArray&amp; …</code></pre>
              …
            </dd>
          </dl>
        </section>

    We scope to `<section id="detailed-description">` and swap the entire
    `<dl>…</dl>` for just what's inside `<dd>` minus its first `<p>`. The
    `<h2>` heading stays put.

    No marker class is required — we find Breathe's `<dl class="cpp class">`
    directly inside the section. Earlier we tried wrapping the directive in
    `<div class="opencv-detail-wrapper">` from the markdown stub, but MyST
    silently drops a bare raw-HTML `<div>` block when a directive sits
    between its opening and closing — the wrapper never reached the rendered
    HTML, the strip never ran, and Breathe's duplicate signature stayed on
    the page. Scoping by section id is wrapper-free and survives that.

    Runs on `build-finished`; idempotent — a re-run finds no `<dl class="cpp
    class">` left (its `<dd>` body has already been hoisted) and no-ops."""
    import re
    if not api_dir.is_dir():
        return
    # Scope to the Detailed Description section so we don't touch other
    # `<dl class="cpp …">` blocks that may live elsewhere on the page.
    section_re = re.compile(
        r'(<section id="detailed-description"[^>]*>)'
        r'(?P<body>[\s\S]*?)'
        r'(</section>)'
    )
    # Breathe's signature `<dl>` for a class.
    dl_re = re.compile(
    ...
    # The trailing "Subclassed by …" Breathe appends after the description.
    # docs.opencv.org shows the inheritance info in the class-info table at
    # the top of the page rather than at the end of the description, so we
    # drop it here. (If you ever want it back, render the inheritance line
    # in the header block alongside the `#include` line.)
    ...
    for h in api_dir.glob("classcv*.html"):
        ...
        def _strip_section(sm):
            ...
            def _strip_dl(dm):
                # Keep the WHOLE <dd> body (including the brief as its first
                # paragraph — docs.opencv.org also opens the Detailed
                # Description with that sentence; the "More..." link in the
                # header is meant to land on it). We only drop the `<dl>`
                # scaffolding wrapping it (signature `<dt>` header) and the
                # trailing "Subclassed by …" paragraph.
...
def _on_build_finished(app, exception):
    """Single build-finished entry point. Each step is small + idempotent so
    they're safe to chain on the same hook."""
